...
/*
 mk_attr_infos：转换属性描述符列表（属性名称，类型和长度）到DataAttrInfo的数组，以便可以将其发送到创建。
 返回：成功列表的长度（> = 0）；否则为错误代码
 */
static int mk_attr_infos(NODE *list, int max, AttrInfo attrInfos[]) {
    int i;
    int list_length;
...
/*
 mk_rel_attrs：转换一系列关系属性（<relation，属性>对）成一个RelAttrs数组
 返回：成功时列表的长度（> = 0）;否则为错误代码
*/
static int mk_rel_attrs(NODE *list, int max, RelAttr relAttrs[]) {
    int i;
...
/*
 parse_format_string：解密格式为xl的格式字符串，其中x是类型说明（“ i” INTEGER，“ r” REAL，“ s” STRING或“ c” STRING（字符）之一），而l是长度 （l对于“ i”和“ r”是可选的），并将类型存储在* type中，将长度存储在* len中。
 返回值：E_OK成功；否则为错误代码
*/
static int parse_format_string(char *format_string, AttrType *type, int *len) {
    int n;
